...
export const useProjectByContractNo = (contractNo: string): ProjectByContractNoResult => {
  ...
  if (listQuery.isError) {
    return { status: 'error', error: listQuery.error, retry: () => { listQuery.refetch() } }
  }
  ...
  if (detailQuery.isError) {
    ...
    return { status: 'error', error: detailQuery.error, retry: () => { detailQuery.refetch() } }
  }
