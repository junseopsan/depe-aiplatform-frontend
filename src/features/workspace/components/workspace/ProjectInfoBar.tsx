...
export const ProjectInfoBar = ({ project }: ProjectInfoBarProps) => {
  ...
  return (
    <section className="flex h-[72px] shrink-0 items-center justify-between gap-6 border-b border-[var(--gray-200)] bg-card px-8">
        ...
        <div className="flex items-center gap-3 text-[13px] tracking-[0.01em] text-[var(--gray-600)] [word-spacing:0.2em]">
          {items.map((item, i) => (
            <span key={item.label} className="inline-flex items-center gap-1.5">
              {i > 0 && (
                <span aria-hidden className="mx-1 text-[var(--gray-400)]">
                  ·
                </span>
              )}
              ...
              <span
                className={
                  item.mono ? "font-mono text-[var(--gray-700)]" : undefined
                }
              >
                {item.value}
              </span>
